...
def build_mlp(input_dim, hidden_dim,output_dim, act='tanh', drop_rate=0.1, bn_first=True, bn_second = False):
    layers = []
    if bn_first:
...
class GAR(BaseColdStartTrainer):
    ...
    def train(self):
        ...
        g_optimizer = torch.optim.Adam(model.generator.parameters(), lr=self.lr, weight_decay=self.args.reg)
        d_optimizer = torch.optim.Adam(model.mlps.parameters(), lr=self.lr, weight_decay=self.args.reg)
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                # Discriminator
                user_emb_r, pos_item_emb_r, neg_item_emb_r, pos_item_emb = model.get_training_embs_r(user_idx, pos_idx, neg_idx)
                with torch.no_grad():
                ...
                rec_loss = (1 - self.args.beta) * rec_loss_adv + self.args.beta * rec_loss_r
                ...
                gen_loss = (1 - self.args.alpha) * gen_loss_adv + self.args.alpha * gen_loss_sim
...
class GAR_Learner(nn.Module):
    def __init__(self, args, data, emb_size, device):
        ...
        self.generator = build_mlp(self.content_dim,192,self.latent_size,drop_rate=0.1,bn_first=False, bn_second=True)
    ...
    def forward(self):
        return self.mlps['user'](self.embedding_dict['user_emb']), self.mlps['item'](self.embedding_dict['item_emb'])
