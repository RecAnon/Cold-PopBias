...
from util.utils import next_batch_pairwise, compress_norms
        ...
        for epoch in range(self.maxEpoch):
            ...
            with torch.no_grad():
                ...
                if self.args.cold_object == 'item':
                    ...
                    now_item_emb = self.model(self.model.cluster_cfmean.to(self.device), self.model.item_content,gen_size=self.data.item_num)
                else:
    ...
    def save(self):
        with torch.no_grad():
            if self.args.cold_object == 'item':
                ...
                now_best_item_emb = self.model(self.model.cluster_cfmean.to(self.device), self.model.item_content,gen_size=self.data.item_num)
            else:
...
class GoRec_Learner(nn.Module):
    ...
    def clustering(self):
        ...
        cluster_fit = KMeans(n_clusters=self.args.pre_cluster_num, random_state=0).fit(self.item_content.cpu().detach().numpy())
        self.cluster_label = cluster_fit.labels_
        cluster_cfmean = []
        for i in range(self.args.pre_cluster_num):
            cluster_cfmean.append(self.embedding_dict[self.args.cold_object+'_emb'][np.where(self.cluster_label[:len(self.data.mapped_warm_item_idx)] == i)].mean(0))
