    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                pos_pair_pred, diff_loss1 = model.heater_encoder_forward(user_idx, pos_idx, True)
                neg_pair_pred, diff_loss2 = model.heater_encoder_forward(user_idx, neg_idx, True)
                batch_pred = torch.cat((pos_pair_pred, neg_pair_pred), dim=0)
                batch_targets = torch.cat((torch.ones_like(pos_pair_pred), torch.zeros_like(neg_pair_pred)), dim=0)
...
class Heater_Learner(nn.Module):
    ...
    def heater_encoder_forward(self, uid, iid, training=True):
        ...
        if self.args.cold_object == 'item':
            ...
            U_embedding, V_embedding, diff_loss, _ = self.heater_encoder.encode(user_emb, item_emb, None, item_content, training)
        else:
    ...
    def forward(self, training=True):
        ...
        if self.args.cold_object == 'item':
            ...
            u_infer_emb, i_infer_emb, _, _ = self.heater_encoder.encode(user_emb, item_emb, None, item_content, training)
        else:
...
class Heater_encoder(nn.Module):
    def __init__(self, latent_rank_in, user_content_rank, item_content_rank, model_select, rank_out, alpha, n_expert, n_dropout):
        ...
        self.experts = nn.ModuleList([nn.Sequential(nn.Linear(self.model_select[-1],self.model_select[-1]),nn.Tanh()) for _ in range(n_expert)])
        self.gate = Gate_func(self.item_content_rank, self.n_expert)
        self.out_linear_u = nn.Sequential(nn.Linear(self.model_select[-1], self.rank_out),
                                          )
        self.out_linear_i = nn.Sequential(nn.Linear(self.model_select[-1], self.rank_out),
                                          )
